...
public class ProofData
{
    ...
    private int totalItems   = 0; // the number of total authentication paths
				// which are introduced by collisions in the
				// process mapping a filename to an index
    private byte[] queryFile = null; // queried filename MAC

    private ArrayList<MetaProofData> authentication = null; // authentication
							    // paths
     ...
     * @param root
     *            - the root Hash value of the authentication tree
     * @return If the returned result from the cloud is correct, return true;
     *         else false.
     */
...
//	for (int i = 0; i < queryFile.length; i = i + 4)
//	    indexExpected = indexExpected + (int) queryFile[i]
//		    + ((int) queryFile[i + 1]) << 8 + ((int) queryFile[i + 2]) << 16 + ((int) queryFile[i + 3]) << 24;
	indexExpected = ((int) queryFile[0]) + (((int) queryFile[1]) << 8) + (((int) queryFile[2]) << 16) + (((int) queryFile[3]) << 24);  // calculate h_1(x)
	...
	//System.out.println("proof data validate cheat flag" + cheatFlag);
	
	indexExpected = ((int) queryFile[4]) + (((int) queryFile[5]) << 8) + (((int) queryFile[6]) << 16) + (((int) queryFile[7]) << 24); // calculate h_2
	indexExpected = Math.abs(indexExpected) % leafSize;
	...
	System.out.println("query file: " + HashTreeBasedOutsourcing.bytes2HexString(this.queryFile));
    ...
    /**
     * This function gets the i-th authentication path, where 'i' starts with
     * index 0.
     * 
     * @param i
     *            - index
